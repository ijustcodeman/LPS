...
public class DWS {
    ...
    public static void main(String[] args) {
        ...
        int suchjahr = 2018; // Suchjahr hier anpassen
        ...
        try (BufferedReader in = new BufferedReader(new FileReader(csvName))) {
            ...
            for (int i = 0; i < records.size(); i++) {
                ...
                for (int j = 0; j < row.length; j++) {
                    if (row[j].equalsIgnoreCase(suchendeSpalte)) {
                        spalte = j;
                        startZeile = i + 1; // Daten beginnen danach
                        break;
                    }
                }
                if (spalte != -1) break;
            ...
            // Alle Kurszeilen des gewünschten Jahres sammeln
            ArrayList<String[]> zeilenImJahr = new ArrayList<>();

            for (int i = startZeile; i < records.size(); i++) {
                String[] row = records.get(i);
                ...
                if (datum.length() >= 10) {
                    try {
                        String jahrStr = datum.substring(6, 10); // "dd.MM.yyyy" -> Jahr
